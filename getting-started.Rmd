...
* [Images](#images-with-markdown) for static images
* [Tables with Markdown](#tables-with-markdown) formatted as PowerPoint
...
By default, the slide level is the highest header level in the hierarchy that is followed immediately by content, and not another header, somewhere in the document. This default can be overridden using the --slide-level option.
...
This presentation sets `slide_level: 3`. The result will be a presentation with three slides: (1) a section header slide; (2) a content slide for the comment, "By default..."; and (3) a title and content slide for the rest of the content.

Without the override this presentation would have been rendered to a single slide. The default rules would set `slide_level: 1` because the section header is immediately followed by content.
...
You can change the slide size and design manually after the PowerPoint presentation is rendered. Because the PowerPoint presentation is rendered with native PowerPoint elements, changing the slide size or design will automatically change titles, content, and tables.

***

# R Code Chunks

[R Code Chunks](https://rmarkdown.rstudio.com/authoring_rcodechunks.html) can be used as a means render R output into documents or to simply display code for illustration. Pandoc supports syntax highlighting for R code chunks.
...
R code chunks can also be used to render plots. Note that images and tables will always be placed on new slides. The only elements that can coexist with an image or table on a slide are the slide header and image/table caption. When you have a text paragraph and an image on the same slide, the image will be moved to a new slide automatically. Images will be scaled automatically to fit the slide.
