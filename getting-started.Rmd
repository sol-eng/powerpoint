...
<p><em>Click <a href="https://github.com/sol-eng/powerpoint">here</a> for a list of examples. </em><em>Submit PowerPoint issues to <a href="https://github.com/rstudio/rmarkdown/issues"><code>rmarkdown/issues</code></a>.</em></p>
</div>

# Getting started

RStudio makes it easy to render PowerPoint presentations from [R Markdown](https://rmarkdown.rstudio.com/) documents. This capability is made possible by recent improvements to [Pandoc](http://pandoc.org/releases.html), `rmarkdown`, and the RStudio IDE. To use all the features of RStudio with PowerPoint output, install the [RStudio Preview](https://www.rstudio.com/products/rstudio/download/preview/) which bundles Pandoc 2.x. You should also install the development version of `rmarkdown`.
...
To put material in side by side columns, you can use a native div container with class `columns`, containing two containers with class `column`. Columns will render using the *Two Content* layout in PowerPoint.
...
4. Two Content. Content slide with two columns.
...
Use Markdown to create [tables](http://pandoc.org/MANUAL.html#tables) that render natively in PowerPoint. These tables recognize PowerPoint table designs and assume the design of whatever template is applied.
...
By default, the slide level is the highest header level in the hierarchy that is followed immediately by content, and not another header, somewhere in the document. This default can be overridden using the `slide-level` option. The document is carved up into slides according to the following [rules](http://pandoc.org/MANUAL.html#structuring-the-slide-show):
...
These rules are designed to support many different styles of slide show. If you don’t care about structuring your slides into sections and subsections, you can just use level 1 headers for each slide. (In that case, level 1 will be the slide level.) But you can also structure the slide show into sections.
...
Capture [HTML Widgets](http://www.htmlwidgets.org/) and [Shiny apps](http://shiny.rstudio.com/) with the [`webshot`](https://cran.r-project.org/web/packages/webshot/vignettes/intro.html) package. With the [`webshot`](https://cran.r-project.org/web/packages/webshot/vignettes/intro.html) package, you can embed a screenshot of a Shiny application that links viewers to a live application hosted on RStudio Connect, Shiny Server, or Shinyapps.io.

Install the [`webshot`](https://cran.r-project.org/web/packages/webshot/vignettes/intro.html) package and [phantomjs](http://phantomjs.org/). The [`webshot`](https://cran.r-project.org/web/packages/webshot/vignettes/intro.html) package will automatically take a screenshot of an interactive HTML Widget and insert it into your presentation. Use `knitr::include_app` to include a Shiny application in your presentation. Viewers can click the image in PowerPoint to link back to the hosted app online.
...
* `cache`. Used to save time by caching plots and other output. If document rendering becomes time consuming due to long computations or plots that are expensive to generate you can use knitr caching to improve performance.
* `screenshot.opts`. Used to delay a webshot. An arbitrary URL may take a long time to load. You can increase the delay to allow apps time to load before taking a webshot.
* `fig.width` and `fig.height`. Used to size a plot. Has to be numeric in inches. Default is 7.
* `fig.cap`. Used to add a caption to an image. Captions will render below the image.

This example code chunks waits one second for an app to load, resizes the width to 15 inches, and caches the image for later use.
...
readLines("getting-started.html") %>%
  .[grep('id="getting-started"', .):(length(.)-31)] %>%
